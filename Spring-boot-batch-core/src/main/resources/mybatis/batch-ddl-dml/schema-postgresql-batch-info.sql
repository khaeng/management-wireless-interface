...
-- DROP TABLE IF EXISTS BATCH_INFO_HST;
...
(
   ...
   JOB_EXEC_ID  NUMERIC(19)  NOT NULL ,
   START_TIME  DATE  ,
   END_TIME  DATE  ,
   STATUS  VARCHAR(10)   ,
   EXIT_CODE  VARCHAR(2500)   ,
   EXIT_MSG  VARCHAR(4000)   ,
   REST_CNT  NUMERIC(9)  DEFAULT 0 NOT NULL  ,
   REST_MSG  VARCHAR(4000)  DEFAULT ''  ,
   LAST_UPDATED  DATE  ,
   PARAMETERS  VARCHAR(200)   ,
   CONSTRAINT BATCH_INFO_HST_PK PRIMARY KEY  ( JOB_ID,KIND_CD,SVR_TYPE_CD,CREATE_TIME )
)
;
...
CREATE TABLE DUAL
(
   DUAL  CHAR(1)  NOT NULL ,
   CONSTRAINT DUAL_DUAL_PK PRIMARY KEY  ( DUAL )
)
;
INSERT INTO dual(dual) VALUES ('A');
